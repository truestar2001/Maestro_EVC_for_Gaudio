...
class EmotionClassifier_Conv1d(nn.Module):
    def __init__(self, layer_num, emb_dim, num_labels, hidden_dim = 100):
        ...
        self.proj = nn.Conv1d(emb_dim, hidden_dim, kernel_size = 1)
    ...
    def forward(self, feature, feature_lens):
        ...
        stacked_features = torch.stack(feature, dim = 0)
        _, *origin_shape = stacked_features.shape
        stacked_features = stacked_features.view(self.layer_num, -1)
        norm_weights = F.softmax(self.weights, dim = -1)
        weighted_features = (norm_weights.unsqueeze(-1) * stacked_features).sum(dim = 0)
        weighted_features = weighted_features.view(*origin_shape)
        
        # downsample embeddings
        weighted_features = torch.permute(weighted_features, (0,2,1))
        downsampled_features = self.proj(weighted_features)
        downsampled_features = torch.permute(downsampled_features, (0,2,1))
        
        # average pooling
        agg_vec_list = []
        for i in range(len(downsampled_features)):
            agg_vec = torch.mean(downsampled_features[i][:feature_lens[i]], dim = 0)
        ...
        #     agg_vec = torch.mean(weighted_features[i][:feature_lens[i]], dim = 0)
        #     agg_vec_list.append(agg_vec)
        
        # avg_emb = torch.stack(agg_vec_list)
        
        # classifier
        final_emb = downsampled_features
        pred = self.out(avg_emb)
        return pred, final_emb



class EmotionClassifier_proj2(nn.Module):
    def __init__(self, layer_num, emb_dim, num_labels, hidden_dim = 100):
        super().__init__()
        self.layer_num = layer_num
        self.emb_dim = emb_dim
...
class EmotionClassifier_new(nn.Module):
    def __init__(self, layer_num, emb_dim, num_labels, hidden_dim = 100):
        super().__init__()
        self.layer_num = layer_num
        self.emb_dim = emb_dim
...
class Emotion_encoder():
    def __init__(self, path):
        ...
        self.clf = EmotionClassifier(layer_num=wavlm_param['layer_num'], emb_dim=wavlm_param['emb_dim'], num_labels=5)
        self.clf.load_state_dict(path)
...
class Trainer():
    def __init__(self, config):
        ...
        self.clf = EmotionClassifier_proj2(layer_num=wavlm_param['layer_num'], emb_dim=wavlm_param['emb_dim'],num_labels=config.num_labels)
        ...
        if args.use_wandb:
            wandb_save_path = "/home/jyp/SpeechEmotionAVLearning/dump/train"
    ...
    def inference(self):
        ...
        save_ESD(self.config.save_path, embeddings, predictions, dataset, status)
...
def save_ESD(dump_path, embeddings, prediction, dataset, status):
    # save_data = {"embeddings":embeddings,"emotion":dataset['emotion'],"intensity":dataset['intensity'], "speaker": dataset['speaker'], "index": dataset['index']}
    save_data = {"audio": dataset['audio'], "embeddings":embeddings, "emotion":dataset['emotion'], "speaker": dataset['speaker'], "text": dataset['text'], "status": status, "prediction": prediction, 'phonemes': dataset['phonemes']}
    with open(os.path.join(dump_path, "embeddings.pickle"), "wb") as f:
        pickle.dump(save_data,f) 
...
if __name__ == "__main__":
    ...
    # Add arguments
    parser.add_argument('--name',type=str, default='train', help="folder name to store the model file")
    parser.add_argument('--lr', type=float, default=1e-5, help='learning rate for optimizer')
    parser.add_argument('--reg-lr', type=float, default=1e-6, help='learning rate for optimizer')
    parser.add_argument('--num-epochs', type=int, default=500, help='number of epochs for training')
    parser.add_argument('--batch-size', type=int, default=32, help='batch size for training')

    parser.add_argument('--data', type=str, default='./generated_ESD_pickle/generated_ESD_audio_train_dataset.pickle', help='path to training data')
    parser.add_argument('--num-labels', type=int, default=5, help='number of categories in data')
    parser.add_argument('--save-path', type=str, default='/home/jyp/SpeechEmotionAVLearning/dump_for_ESD_EVC_proj2', help='path to save trained model')
    ...
    # parser.add_argument('--load-path', type=str, default='./dump/ESD_train_for_EVC/model_best.pth', help='path to load pretrained model')
